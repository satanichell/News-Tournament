...
public partial class User_Questions_Edit : System.Web.UI.Page
{
    ...
    private void QuestionsAndAnswer()
    {
        ...
        while (DataReader.Read())
        {
            ...
            string s = UserAnswerInDB(DataReader["QMOS_ID"].ToString());

            if (s != "0")
            {
                rblGroup.SelectedValue = s;
            }
    ...
    private void SetUserAnswerInDB(Dictionary<string, string> Array1)
    {
        ...
        foreach (var pair in Array1)
        {
            ...
            SqlCommand Command = new SqlCommand("UPDATE [tbl_AnswerQuestion] SET [AQ_UserAnswer] = @UserAnswer WHERE ([AQ_UserID] = @UserID) AND ([AQ_DateMOSID] = @MosID) AND ([AQ_QuestionID] = @QuestionID)", Connection);
    ...
    private string UserAnswerInDB(string QuestionID)
    {
        string UserID = Session["ID"].ToString();
        string MosID = Request.Params["ID"].ToString();
        ...
        SqlCommand Command = new SqlCommand("SELECT [AQ_UserAnswer] FROM [tbl_AnswerQuestion] WHERE ([AQ_UserID] = @UserID) AND ([AQ_DateMOSID] = @MosID) AND ([AQ_QuestionID] = @QuestionID)", Connection);
        Command.Parameters.AddWithValue("@UserID", UserID);
        Command.Parameters.AddWithValue("@MosID", MosID);
        Command.Parameters.AddWithValue("@QuestionID", QuestionID);
        ...
        SqlDataReader DataReader = Command.ExecuteReader();
        DataReader.Read();
        Answer = DataReader["AQ_UserAnswer"].ToString();
